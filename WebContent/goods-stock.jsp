							...
							<table class="table table-bordered table-striped">
								<thead>
									<tr>
										...
										<th>生产日期</th>
										<th>保质期</th>
										<th class="hidden-phone">采购价格</th>
										<th class="hidden-phone">采购数量</th>
										<th >确定采购</th>
										
									</tr>
								</thead>
								<c:forEach items="${requestScope.pgoods}" var="pgood">
									<tr class="gradeX">
										<td>${pgood.name}</td>
										<td>${pgood.count}</td>
										<td>${pgood.providerName}</td>
										<td>${pgood.productDate}</td>
										<td>${pgood.saveDate}</td>
										<td class="center hidden-phone">${pgood.providerprice}</td>
						<form action="${pageContext.request.contextPath}/DoStockServlet" method="get">
										<td class="center hidden-phone"><input type="number" value="0" name="count" ></td>
										<input type="hidden" value="${pgood.id}" name="id">
