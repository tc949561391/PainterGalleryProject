				...
				<div class="panel panel-default bk-bg-white">
					...
					<div class="panel-body">
						<form action="${pageContext.request.contextPath}/UserDeleteservlet" method="post">
							<table class="table table-bordered table-striped">
								...
								<c:forEach items="${requestScope.users}" var="usero">
									<tr class="gradeX">
										...
										<td><c:if test="${sessionScoper.user.manager==1}">${usero.password}</c:if> <c:if
											test="${sessionScoper.user.manager!=1}">**********</c:if></td>
										<td><c:if test="${usero.manager==1}">管理员</c:if> <c:if
												test="${usero.manager==0}">普通用户</c:if></td>
										<td class="center hidden-phone"><input type="checkbox"
											name="delete_${usero.userName}"  /></td>
									</tr>
								</c:forEach>
							</table>
							...
							<c:if test="${sessionScope.user.manager!=1}">
							<h1 style="color: red;font: small-caps;">您不是管理员，无权删除</h1>
							</c:if>
						</form>
					</div>
				</div>
