				...
				<div class="panel panel-default bk-bg-white">
					...
					<div class="panel-body">
						<form
							...
							<table class="table table-bordered table-striped">
								<thead>
									<tr>
										...
										<th>密码</th>
										<th>权限</th>
										<th class="hidden-phone">授权</th>
									</tr>
								</thead>

								<c:forEach items="${requestScope.users}" var="usero">
									<tr class="gradeX">
										<td>${usero.userName}</td>
										<td><c:if test="${sessionScoper.user.manager==1}">${usero.password}</c:if>
										 <c:if test="${sessionScoper.user.manager!=1}">**********</c:if></td>
										<td><c:if test="${usero.manager==1}">管理员</c:if> <c:if
												test="${usero.manager==0}">普通用户</c:if></td>
										<td><select name="${usero.userName}">
												<option value="manager">管理员</option>
												<option value="normal">普通用户</option>
												<option value="none" selected="selected"><c:if
														test="${usero.manager==1}">管理员</c:if>
													<c:if test="${usero.manager==0}">普通用户</c:if></option>
										</select></td>
									</tr>
								</c:forEach>
							</table>
							...
							<c:if test="${sessionScope.user.manager!=1}">
								<h1 style="color: red; font: small-caps;">您不是管理员，无权授权</h1>
							</c:if>
						</form>
					</div>
				</div>
